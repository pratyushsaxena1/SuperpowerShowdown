...
-- then deal a small damage hit on contact. Strong "anti-cover" tool —
...
Magnetism.pullDuration = 0.45
Magnetism.pullDamage = 14
Magnetism.contactRadius = 5
Magnetism.range = 60
...
function Magnetism.onActivate(character, opponent, ctx)
	...
	local pullVec = (selfRoot.Position - oppRoot.Position).Unit * 80
		+ Vector3.new(0, 18, 0)
	...
	-- Contact damage: after a short delay, if the opponent is now in
	-- contact range, deal pullDamage. Stops chip-spam if the opponent
	-- got blocked by geometry on the way in.
	task.delay(Magnetism.pullDuration + 0.05, function()
		if not (opponent.Parent and selfRoot.Parent) then return end
		local nowDist = (oppRoot.Position - selfRoot.Position).Magnitude
		if nowDist <= Magnetism.contactRadius and ctx and ctx.dealDamage then
